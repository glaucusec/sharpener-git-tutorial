    ...
    document.querySelector('#logout').addEventListener('click', () => {
        ...
        window.location = 'http://localhost:80/user/login';
    })
    ...
    function getoldURLs(page) {
        ...
        axios.get(`http://localhost:80/premium/fileurls?page=${page}&limit=${limit}`, {
            headers: {
                'Authorization': token
            }
        })
        ...
        // try {
        //     let prevFiles = await axios.post('http://localhost:80/premium/fileurls', {}, {
        //         headers: {
    ...
    document.querySelector('#report-button').addEventListener('click', async (e) => {
        e.preventDefault();
        const token = localStorage.getItem('token');
        axios.get('http://localhost:80/premium/download-report', {
            headers: {
                'Authorization': token
            }
        })
    ...
    document.querySelector('#leaderboard-button').addEventListener('click', async (e) => {
        e.preventDefault();
        await axios.post('http://localhost:80/premium/leaderboard', {}, {
            headers: {
                "Authorization": token
            }
        })
    ...
    async function hideOrNot () {
        await axios.post('http://localhost:80/purchase/ispremium', null, {
            headers: {
                "Authorization" : token
            }
        })
    ...
    rzpButton.addEventListener('click', async (e) => {
        ...
        const token = localStorage.getItem('token');
        const response = await axios.post('http://localhost:80/purchase/premium', null, {
            headers: {
                "Authorization" : token
            }
        })
        ...
        let options = {
            ...
            "handler": async function(res) {
                const token = localStorage.getItem('token');
                await axios.post('http://localhost:80/purchase/updatetransactionstatus', {
                    success: true,
                    ...
                    window.location.href = 'http://localhost:80/user/daily-expenses'
    ...
    formEl.addEventListener('click', (e) => {
        ...
        axios.post('http://localhost:80/user/expenses', formData, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': token
            }
        })
        .then(window.location.href = 'http://localhost:80/user/daily-expenses');
    });
    ...
    async function fetchExpenses() {
        const token = localStorage.getItem('token');
        let response = await axios.get('http://localhost:80/user/expenses', {
            headers: {
                "Authorization": token
            }
        });
